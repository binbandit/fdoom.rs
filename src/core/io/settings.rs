...
impl Settings {
    pub fn new(loc: &Localization) -> Settings {
        ...
        put(
            "size",
            ArrayEntry::new("World Size", vec![128.into(), 256.into(), 512.into()], loc),
        );
        put(
            "theme",
            ArrayEntry::new(
        ...
        settings
            .get_entry("scoretime")
            .borrow_mut()
            .set_value_visibility(&Value::Int(10), false);
        settings
            .get_entry("scoretime")
            .borrow_mut()
            .set_value_visibility(&Value::Int(120), false);
        settings
            .get_entry("language")
            .borrow_mut()
        ...
        // Java change actions (closures over the shared entries):
        {
            ...
            settings
                .get_entry("mode")
                .borrow_mut()
                .set_change_action(Box::new(move |value| {
                    scoretime
                        .borrow_mut()
                        .set_visible(matches!(value, Value::Str(s) if s == "Score"));
                }));
        }
        {
            let skinon = settings.get_entry("skinon");
            settings
                .get_entry("unlockedskin")
                .borrow_mut()
                .set_change_action(Box::new(move |value| {
    ...
    pub fn get(&self, option: &str) -> Value {
        self.options[&option.to_lowercase()]
            .borrow()
            .get_value()
            .clone()
    }
