...
//!
//! Java stores `Tile` references and `Furniture` prototypes; the port stores interned
//! [`TileId`]s (resolved through `Tiles` at draw time) and furniture constructor
//! functions (Java `furniture.clone()` becomes constructing a fresh entity). Java's
//! `HashSet<TilePoint>` (equality on x, y, tile id) becomes a `Vec` with the same
//! duplicate rejection; the set's iteration order is unspecified in Java, but no
//! structure places two different tiles at one point, so order does not affect the
//! result.
...
use crate::level::tile::{TileId, Tiles, dispatch, ids};

/// Java `Structure.TilePoint`.
struct TilePoint {
    x: i32,
    y: i32,
    /// Interned tile id (Java stored the `Tile` itself).
    tile: TileId,
}
...
pub fn dungeon_gate() -> Structure {
    ...
    s.set_tile(-1, 0, ids::OBSIDIAN);
    s.set_tile(1, 0, ids::OBSIDIAN);
    s.set_tile(2, 0, ids::OBSIDIAN_DOOR);
    s.set_tile(-2, 0, ids::OBSIDIAN_DOOR);
    s.set_tile(0, -1, ids::OBSIDIAN);
    s.set_tile(0, 1, ids::OBSIDIAN);
    s.set_tile(0, 2, ids::OBSIDIAN_DOOR);
    s.set_tile(0, -2, ids::OBSIDIAN_DOOR);
    s.set_tile(-1, -1, ids::OBSIDIAN);
    s.set_tile(-1, 1, ids::OBSIDIAN);
    s.set_tile(1, -1, ids::OBSIDIAN);
    s.set_tile(1, 1, ids::OBSIDIAN);
    s.set_tile(3, 0, ids::OBSIDIAN);
    s.set_tile(-3, 0, ids::OBSIDIAN);
    s.set_tile(3, -1, ids::OBSIDIAN);
    s.set_tile(-3, -1, ids::OBSIDIAN);
    s.set_tile(3, 1, ids::OBSIDIAN);
    s.set_tile(-3, 1, ids::OBSIDIAN);
    s.set_tile(4, 0, ids::OBSIDIAN);
    s.set_tile(-4, 0, ids::OBSIDIAN);
    s.set_tile(4, -1, ids::OBSIDIAN);
    s.set_tile(-4, -1, ids::OBSIDIAN);
    s.set_tile(4, 1, ids::OBSIDIAN);
    s.set_tile(-4, 1, ids::OBSIDIAN);
    s.set_tile(0, 3, ids::OBSIDIAN);
    s.set_tile(0, -3, ids::OBSIDIAN);
    s.set_tile(1, -3, ids::OBSIDIAN);
    s.set_tile(-1, -3, ids::OBSIDIAN);
    s.set_tile(1, 3, ids::OBSIDIAN);
    s.set_tile(-1, 3, ids::OBSIDIAN);
    s.set_tile(0, 4, ids::OBSIDIAN);
    s.set_tile(0, -4, ids::OBSIDIAN);
    s.set_tile(1, -4, ids::OBSIDIAN);
    s.set_tile(-1, -4, ids::OBSIDIAN);
    s.set_tile(1, 4, ids::OBSIDIAN);
    s.set_tile(-1, 4, ids::OBSIDIAN);
    s.set_tile(-2, -2, ids::OBSIDIAN_WALL);
    s.set_tile(-3, -2, ids::OBSIDIAN_WALL);
    s.set_tile(-3, 2, ids::OBSIDIAN_WALL);
    s.set_tile(-2, 1, ids::OBSIDIAN_WALL);
    s.set_tile(2, -2, ids::OBSIDIAN_WALL);
    s.set_tile(4, -2, ids::OBSIDIAN_WALL);
    s.set_tile(4, 2, ids::OBSIDIAN_WALL);
    s.set_tile(-4, -2, ids::OBSIDIAN_WALL);
    s.set_tile(-4, 2, ids::OBSIDIAN_WALL);
    s.set_tile(1, -2, ids::OBSIDIAN_WALL);
    s.set_tile(-2, 2, ids::OBSIDIAN_WALL);
    s.set_tile(2, 3, ids::OBSIDIAN_WALL);
    s.set_tile(2, 4, ids::OBSIDIAN_WALL);
    s.set_tile(-2, -3, ids::OBSIDIAN_WALL);
    s.set_tile(-2, -4, ids::OBSIDIAN_WALL);
    s.set_tile(2, -3, ids::OBSIDIAN_WALL);
    s.set_tile(2, -4, ids::OBSIDIAN_WALL);
