...
fn new_mob_instance(g: &Game, template: &Entity, lvl: i32) -> Option<Entity> {
    Some(match &template.kind {
        ...
        EntityKind::Slime(_) => mob::slime::new(g, lvl),
        EntityKind::Creeper(_) => mob::creeper::new(g, lvl),
        EntityKind::Skeleton(_) => mob::skeleton::new(g, lvl),
        EntityKind::Snake(_) => mob::snake::new(g, lvl),
        EntityKind::Knight(_) => mob::knight::new(g, lvl),
        // JAVA: AirWizard(int lvl) delegates to AirWizard(lvl > 1)
        EntityKind::AirWizard(_) => mob::air_wizard::new(g, lvl > 1),
        _ => return None,
    })
}
