...
The art is split PNG files under `assets/sprites/**` — one file per sprite, and **the
files are the source of truth** (edit them with `just studio` or any pixel editor).
They are stitched into one atlas at load time (`src/gfx/sprite_sheet.rs`; dev runs read
the folder live, release builds use a build-time embedded copy — `src/assets.rs`).
Full authoring guide: docs/ART_GUIDE.md.
...
- The atlas is hybrid: gray pixels (`r==g==b`) are quantized to **4 shades** on load
  (`value / 64` → 0..3) and recolored per draw call; any non-gray pixel is true color,
  drawn literally. `0/85/170/255` are the four canonical grays (enforced for
  `pal`-flagged files by `tests/sprite_atlas.rs`).
...
- The base atlas is 256x256 (32x32 cells); `pos = x + y*32` addresses all of it. The
  pin table (`assets/sprites/manifest.txt`) is the authoritative cell map. **New
  sprites don't need cells picked at all** — drop a PNG in the tree and it
  auto-allocates onto appended rows, addressed by name via `sheet.cell("items/foo")`
  (docs/ART_GUIDE.md has the end-to-end recipe). (The legacy Java sheets `icons.png` /
  `icons_ale.png` / `logo.png`, and later the artgen generator, were retired once
  everything worth keeping was traced into the sprite tree — see git history.)
