...
**Sprites are colored at draw time, not on the sheet.** The art lives as split PNG
files under `assets/sprites/**` (one file per sprite, the source of truth — see
docs/ART_GUIDE.md), stitched into a single hybrid atlas of 8x8 cells at load time
(`src/gfx/sprite_sheet.rs`): gray pixels are quantized to one of 4 recolorable shades
(0=darkest, 3=lightest), while non-gray pixels are true color drawn literally. Every render call takes a `colors: i32` produced
...
| Sprites / colors / font | `src/gfx/` (`sprite.rs`, `color.rs`, `font.rs`), art at `assets/sprites/**` (docs/ART_GUIDE.md) |
