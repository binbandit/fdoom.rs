...
**Sprites are colored at draw time, not on the sheet.** `assets/sprites.png`
(generated by `cargo run --bin artgen` — never hand-edited) is a hybrid sheet of 8x8
cells: gray pixels are quantized to one of 4 recolorable shades (0=darkest, 3=lightest,
`src/gfx/sprite_sheet.rs`), while non-gray pixels are true color drawn literally. Every render call takes a `colors: i32` produced
...
| Sprites / colors / font | `src/gfx/` (`sprite.rs`, `color.rs`, `font.rs`), sheet at `assets/sprites.png` (from `src/bin/artgen.rs`) |
